...
# =====NETWORK CONNECTIONS====
#Send the network connection logs to a network.log the filters applied are: 
#
# Doesn't record INET6 Connections, IP v4 connections starting with 10.*, 192.*, 0.* or 255.*

supraudit  -S -F net /dev/auditpipe | grep -Ev 'ioctl|close\(|INET6|127.0.0.1|INET4 10.|INET4 192.|INET4 0.|INET4 255.' >> /var/log/supraudit/network.log 
